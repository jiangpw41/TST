    ...
    for tgt in tgt_data:
        ...
        for pred in pred_data:
            ...
            try:
                ...
                raise Exception(f"Target {tgt}; Pred {pred}")
...
def eval_main( dataset, loaded_pair_list, results_save_path, eval_type="multi_entity"):
    ...
    for metric in score_s:
        ...
        for hyp_table, tgt_table in tqdm(zip( loaded_pair_list[1], loaded_pair_list[0]), total=len(loaded_pair_list[1])):
            #两者都存在，则计算指标
            ...
            if row_header:
                if len(hyp_table)==0 and len(tgt_table)!=0:
                    p, r, f = 0, 0, 0
                elif len(hyp_table)==0 and len(tgt_table)==0:
                    p, r, f = 1, 1, 1
                else:
    ...
    data = load_data( results_save_path, "json")
    data[ f"{dataset}"+f"_{human_readable_time}" ] = resutls_total     # 以键值对保存结果
    save_data( data, results_save_path)
