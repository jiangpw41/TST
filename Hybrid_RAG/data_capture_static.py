...
class RatioTextCapture( TextCapture ):
    ...
    """
    def __init__(self, text: str, context_length = 10, orient = "all"):
        self.match_word = re.compile( r'(?<!\d)\d{1,2}(\.\d{1,2})?%' )
        super().__init__( text, self.match_word, context_length, orient)
        self.dict = {}
        self.all_capture()
...
if __name__=="__main__":
 ...
 '原告在本院指定的举证期限内提供了如下证据：1.原告身份证，证明原告诉讼主体资格；2.被告人口信息、身份证，证明被告诉讼主体资格；3.借据，证明被告向原告借款及约定借款利率的事实。']
    ...
    text = ['原告陈美绿起诉称：2014年5月17日，被告陈爱慈因资金周转所需向原告借款130000元。后因双方发生矛盾，原告多次要求被告偿还借款均无果。故原告起诉要求判决：被告返还借款130000元及利息（从借款之日起按月利率1%计算至实际还款之日止），并承担本案诉讼费。',
 '原告陈美绿在庭审中陈述：涉案借款约定月利率为1%。被告至今未偿还借款本息。']
    capture = CplDocumentRatioTextCapture( text, context_length=30 )    
    capture.get_related_context( '第1次约定的利息的利率数值（百分比或元）' )